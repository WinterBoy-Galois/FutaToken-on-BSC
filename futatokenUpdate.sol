...
contract FUTAINU is Context, IERC20, Ownable {
    ...
    string private _name     = "FUTAINU";
    string private _symbol   = "FUTA";
    ...
    uint256 private _swapFee = _liquidityFee + _treasuryFee;
    
    uint256 public  _maxTxAmount     = 1000 * 10**9 * 10**18;
    uint256 private _minTokenBalance = 3000000 * 10**18;
    uint256 public  _maxWalletSize = 1000 * 10**9 * 10**18;
    ...
    event TreasuryFeeSent(address to, uint256 ethSent);
    ...
    constructor (address cOwner, address treasuryWallet) Ownable(cOwner) {
        ...
        _isExcludedFromMxWalletSize[_treasuryWallet] = true;
    ...
    function updateFees(uint256 taxFee, uint256 liquidityFee, uint256 treasuryFee) external onlyOwner {
        uint256 _totalFees = taxFee + liquidityFee + treasuryFee;
        require(_totalFees <=15, "Must Keep Fees at 15% or less" );

        _taxFee = taxFee;
        _liquidityFee = liquidityFee;
        _treasuryFee = treasuryFee;
        _swapFee = _liquidityFee + _treasuryFee;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 treasuryFeeToken = contractTokenBalance.mul(_treasuryFee).div(_swapFee);

        uint256 half      = (contractTokenBalance.add(treasuryFeeToken)).div(2);
        ...
        */
        ...
        // take marketing fee
        uint256 treasuryFeeETH = newBalance.mul(treasuryFeeToken).div(half);
        uint256 ethForLiquidity = newBalance.sub(treasuryFeeETH);
        if (treasuryFeeETH > 0) {
            payable(_treasuryWallet).transfer(treasuryFeeETH);
            emit TreasuryFeeSent(_treasuryWallet, treasuryFeeETH);
        }
